...
func integTest_getSecretStringWithTTL(t *testing.T, api *secretsmanager.Client) string {
	...
	resultString, _ = cache.GetSecretString(context.Background(), *createResult.ARN)
...
func integTest_getSecretStringNoSecret(t *testing.T, api *secretsmanager.Client) string {
	...
	if err == nil {
		...
		if errors.As(err, &rnf) {
			...
			t.Errorf("failed to call service: %s, operation: %s, error: %v", oe.Service(), oe.Operation(), oe.Unwrap())
		}
	}
